...
#define PH1_ADDRESS       0x14
#define PH2_ADDRESS       0x17
#define OXYGEN_ADDRESS    0x0A
#define TURBIDITY_ADDRESS 0x28
 ...
 */
const struct smart_sensor_driver smart_sensor_driver_ponsel = {
...
static int prepare(struct smart_sensor *sensor)
{
    ...
    prepare_modbus_frame(&f, sensor, MODBUS_WRITE_SINGLE_HOLDING_REGISTER, 0x0001, 0x001F);
...
static int detect(int sensor_number, struct smart_sensor *sensor)
{
    ...
    DEBUG("Checking Ponsel %i... ", sensor_number);
    for (tries = 0; tries < DETECTION_TRIES; tries++) {
        sensor->number = sensor_number;
        if (prepare(sensor) == 0) {
            sensor->manufacturer = PONSEL;
            sensor->power_up_time = PONSEL_SENSORS_POWERUP_TIME;
            sensor->type = PH_SENSOR;
            sensor->channel = 0;
            strcpy(sensor->name, "PONSEL");
            DEBUG("OK\n");
            return 1;
        } else {
            DEBUG("NO\n");
                                 ...
                                 uint16_t coils)
{
    switch (s->number) {
        case 0:
            f->slave_address = PH1_ADDRESS;
            break;
        case 1:
            f->slave_address = PH2_ADDRESS;
            break;
        case 2:
            f->slave_address = TURBIDITY_ADDRESS;
            break;
        case 3:
            f->slave_address = OXYGEN_ADDRESS;
            break;
        default:
            f->slave_address = 1;
            break;
...
static int modbus_request_measurement(struct smart_sensor *sensor, struct measurement *measurement)
{
    ...
    /*READ THE 6 REGISTERS FOR TEMP, PH AND REDOX*/
    prepare(sensor);
    watchdog_disable();
    ...
    /* READ TEMP */
    if (sensor->number == 3) {
        prepare_modbus_frame(&f, sensor, MODBUS_READ_HOLDING_REGISTERS, 0x0053, 8);
    } else {
        prepare_modbus_frame(&f, sensor, MODBUS_READ_HOLDING_REGISTERS, 0x0053, 4);
    }
    modbus_query(UART_SMART_SENSOR, &f);
    response_status = modbus_poll(UART_SMART_SENSOR, &f, BIG_ENDIAN);
    if (response_status == -E_NOT_DETECTED) {
    ...
    if ((f.slave_address == PH1_ADDRESS) || (f.slave_address == PH2_ADDRESS)) {
        DEBUG("PONSEL TEMP: %.2f pH: %.2f\n", (double)param1, (double)param2);
        ...
        m->redox_status = MEASUREMENT_OK;
    } else if (f.slave_address == TURBIDITY_ADDRESS) {
        DEBUG("PONSEL TEMP: %.2f NTU: %.2f\n", (double)param1, (double)param2);
    ...
    } else if (f.slave_address == OXYGEN_ADDRESS) {
        DEBUG("PONSEL TEMP: %.2f Oxy: %.2f %% Oxy: %.2f mg/L\n", (double)param1, (double)param2, (double)param3);
        ...
        m->salinity_status = MEASUREMENT_VALUE_FIXED;
    }
    return 1;
}
