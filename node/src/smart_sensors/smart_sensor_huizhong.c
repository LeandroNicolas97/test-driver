...
/*
 * Communication with the smart sensors Huizhong Ultrasonic Flow
 */
 ...
 */
const struct smart_sensor_driver smart_sensor_driver_huizhong = {
...
{
    DEBUG("Checking Huizhong Flow %i... ", sensor_number);
    for (int tries = 0; tries < DETECTION_TRIES; tries++) {
        sensor->number = sensor_number;
        if (prepare(sensor) == 0) {
            sensor->manufacturer = HUIZHONG;
            sensor->power_up_time = HUIZHONG_POWERUP_TIME;
...
static int modbus_request_measurement(struct smart_sensor *sensor, struct measurement *measurement)
{
    ...
    /*     return rc; */
    /* } */
    /* DEBUG("HUIZH TEMPERATURE FLOW: %.2f\n", temperature); //m3 */
